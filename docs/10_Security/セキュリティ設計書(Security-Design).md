...
| 項目 | デフォルト値 | 設定範囲 | 実装状況 |
|------|-------------|---------|----------|
| 最小文字数 | 8文字 | 6-32文字 | ✅ 実装済み |
| 大文字必須 | 有効 | 有効/無効 | ✅ 実装済み |
| 数字必須 | 有効 | 有効/無効 | ✅ 実装済み |
| 特殊文字必須 | 無効 | 有効/無効 | ✅ 実装済み |
| 有効期限 | 90日 | 0-365日（0は無期限） | ⚠️ Phase 5予定 |
| パスワードハッシュ | bcrypt推奨 | - | ✅ 実装済み（128ビットsalt） |
...
| 項目 | デフォルト値 | 説明 | 実装状況 |
|------|-------------|------|----------|
| 失敗許容回数 | 5回 | ロックまでの失敗回数 | ✅ 実装済み（WebUI-Sample） |
| アカウントロック時間 | 15分 | ロック解除までの時間 | ✅ 実装済み（WebUI-Sample） |
| IPブロック | 無効 | 同一IPからの試行制限 | ⚠️ Phase 5予定 |
| レート制限 | 10req/分 | RateLimiterによる制限 | ✅ 実装済み（security.js） |
...
|--------|-----------|:----:|---------|-------------|
| CVE-001 | パスワードハッシュのsalt不足 | 7.5 (高) | 128ビットランダムsaltを導入、"hash:salt"形式に変更、後方互換性を維持 | `auth.js` |
| CVE-002 | デモ認証情報のハードコード | 9.1 (緊急) | admin/admin123, demo/demo等のハードコード認証情報を完全削除、本番環境向け実装のみ残存 | `auth.js` |
| CVE-003 | IPアドレスのシミュレーション値 | 4.3 (中) | 固定値 '192.168.1.100' を削除、'N/A (ブラウザ側では取得不可)' に変更、サーバー側実装を推奨 | `modules.js` |
| CVE-004 | localStorage平文保存 | 6.5 (中高) | 機密情報フィルタリング関数を追加、パスワード/APIキー/トークンを自動マスキング、HTTPS警告出力、IndexedDB暗号化への移行を推奨 | `modules.js`, `api.js` |
...
|-----------|------|----------|--------|
| 1.0 | 2026-01-20 | 初版作成 | - |
| 2.0 | 2026-02-04 | CVE-001~004修正内容追加、ブルートフォース対策実装状況更新、CSRF対策予定明記、セキュリティスコア95点更新 | Claude Code |
