...
(() => {
  ...
  const state = {
    ...
    users: [
      { name: "加藤", dept: "システムG", role: "管理者", risk: "低", lastLogin: "2026-01-14" },
      { name: "山田", dept: "総務部", role: "業務管理者", risk: "中", lastLogin: "2026-01-13" },
      { name: "佐藤", dept: "工事部", role: "利用者", risk: "低", lastLogin: "2026-01-12" },
      { name: "鈴木", dept: "経理部", role: "利用者", risk: "低", lastLogin: "2026-01-10" },
      { name: "田中", dept: "工事部", role: "利用者", risk: "高", lastLogin: "2025-11-02" }, // stale
    ],
    deletions: [
  ...
  function badgeClass(level){
    if (level === "高" || level === "WARN") return "warn";
    if (level === "OK" || level === "低") return "good";
  ...
  const views = {
    dashboard(){
      ...
      view.innerHTML = `
        <div class="grid">
          ...
          <div class="card" style="grid-column: span 3;">
            <div class="kpi">
              <div class="kpi__icon green"><i class="fa-solid fa-pen-to-square"></i></div>
              <div class="kpi__text">
                <div class="kpi__num">${k.changesMonth}</div>
                <div class="kpi__label">今月の更新件数</div>
              </div>
            </div>
          </div>
          ...
          <div class="card" style="grid-column: span 3;">
            <div class="kpi">
              <div class="kpi__icon orange"><i class="fa-solid fa-trash-can"></i></div>
              <div class="kpi__text">
                <div class="kpi__num">${k.deletesPending}</div>
                <div class="kpi__label">削除申請（未処理）</div>
              </div>
            </div>
          </div>
          ...
          <div class="card" style="grid-column: span 5;">
            ...
            <div style="margin-top:12px; display:grid; gap:10px;">
              ...
              <div class="card" style="box-shadow:none; background:rgba(255,255,255,.6);">
                <div class="toolbar">
                  <div><i class="fa-solid fa-link" style="color:#16a34a;"></i> 連携アラート</div>
                  <span class="badge ${k.integrationAlerts ? "warn" : "good"}">${k.integrationAlerts} 件</span>
                </div>
              </div>
              <div class="card" style="box-shadow:none; background:rgba(255,255,255,.6);">
                <div class="toolbar">
                  <div><i class="fa-solid fa-user-shield" style="color:#8b5cf6;"></i> 権限見直し</div>
                  <span class="badge warn">今週の対象: 2</span>
                </div>
              </div>
            </div>
          </div>
    ...
    incidents(){
      setHeader("障害・インシデント管理", "誤操作・連携障害を“学び”に変える");
      view.innerHTML = `
        <div class="card">
          <div class="card__head">
            <div>
              <div class="card__title"><i class="fa-solid fa-triangle-exclamation"></i>インシデント一覧（例）</div>
    ...
    audits(){
      setHeader("定期点検・棚卸", "点検を“イベント”で終わらせない");
      view.innerHTML = `
        <div class="grid">
          <div class="card" style="grid-column: span 6;">
            <div class="card__head">
              <div>
                <div class="card__title"><i class="fa-solid fa-clipboard-check"></i>点検メニュー</div>
                <div class="card__meta">棚卸は“運用の健康診断”</div>
              </div>
            </div>

            <table class="table">
              <thead><tr><th>点検</th><th>期限</th><th>状態</th></tr></thead>
              <tbody>
                ${state.audits.map(a => `
                  <tr>
                    <td>${esc(a.name)}</td>
                    <td>${esc(a.due)}</td>
                    <td><span class="badge ${a.status.includes("未")?"warn":"good"}">${esc(a.status)}</span></td>
                  </tr>
                `).join("")}
              </tbody>
            </table>
          ...
          <div class="card" style="grid-column: span 6;">
            <div class="card__head">
              <div>
                <div class="card__title"><i class="fa-solid fa-list-check"></i>チェックリスト（例）</div>
                <div class="card__meta">最小セット（運用に効く順）</div>
              </div>
            </div>

            <div style="display:grid; gap:10px;">
              ${[
                ["削除権限はIT部門のみになっている", "fa-trash-can", "warn"],
                ["異動・退職ユーザーの権限が剥奪されている", "fa-user-xmark", "warn"],
                ["重要台帳のバックアップが期限内", "fa-box-archive", "good"],
                ["連携エラーが放置されていない", "fa-link-slash", "good"],
              ].map(([t, icon, cls]) => `
                <div class="card" style="box-shadow:none; background:rgba(255,255,255,.6); display:flex; gap:10px; align-items:center;">
                  <i class="fa-solid ${icon}" style="color:${cls==="warn" ? "#f59e0b" : "#16a34a"};"></i>
                  <div style="font-weight:800; flex:1;">${esc(t)}</div>
                  <span class="badge ${cls}">${cls==="warn"?"要確認":"OK"}</span>
                </div>
              `).join("")}
            </div>
          </div>
