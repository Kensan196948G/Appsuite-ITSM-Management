...
| LAN IP | 192.168.0.185 |
| 開発環境ポート | 3000 (HTTP) |
| 本番環境ポート | 8443 (HTTPS) |
| 開発環境URL | http://localhost:3000 (ローカル)<br>http://192.168.0.185:3000 (LAN) |
| 本番環境URL | https://localhost:8443 (ローカル)<br>https://192.168.0.185:8443 (LAN) |
...
# 本番環境用
openssl req -x509 -newkey rsa:4096 -keyout ssl/prod-key.pem -out ssl/prod-cert.pem -days 365 -nodes -subj "/CN=192.168.0.185"
```
...
Write-Host "   LAN: http://192.168.0.185:$PORT" -ForegroundColor Cyan
...
Write-Host "   LAN: https://192.168.0.185:$PORT" -ForegroundColor Cyan
...
| 開発（LAN） | http://192.168.0.185:3000 | [開発] AppSuite ITSM (LAN) |
| 本番（ローカル） | https://localhost:8443 | [本番] AppSuite ITSM (localhost) |
| 本番（LAN） | https://192.168.0.185:8443 | [本番] AppSuite ITSM (LAN) |
